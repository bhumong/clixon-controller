...
/*! Given mount-point domain/spec and api_path_fmt, find api_path
 *
 * @param[in]  h            Clixon handle
 * @param[in]  cvv          Vector of cli string and instantiated variable
 * @param[in]  domain       Domain of the mount-point (NULL for root)
 * @param[in]  spec         yspec name of the mount-point (NULL for root)
               ...
               cvec               *nsctx)
{
    ...
    for (i=0; i<cvec_len(argv); i++){
        cv = cvec_i(argv, i);
        str = cv_string_get(cv);
        if (str != NULL && strncmp(str, MTPOINT_PREFIX, strlen(MTPOINT_PREFIX)) == 0){
            mtpoint = str;
